...
# ----------------------- Server Class -----------------------
class Server:
    def __init__(self, num_clients, num_adversaries, num_data_per_client, device):
        self.num_clients = num_clients
        self.num_adversaries = num_adversaries
        self.num_data_per_client = num_data_per_client
        self.device = device
        self.clients = [Client(i, device) for i in range(num_clients)]

    def distribute_data(self, dataloader, model_resnet):
        for i, (real_image, label) in enumerate(dataloader):
            if i >= self.num_clients * self.num_data_per_client:
                break
            client_id = i // self.num_data_per_client
            real_image, label = real_image.to(self.device), label.to(self.device)
            if client_id < self.num_adversaries:
                fake_image = deep_leakage_reconstruct(real_image, model_resnet, label, self.device)
                self.clients[client_id].add_data(fake_image.cpu(), 0)  # Fake
            else:
                self.clients[client_id].add_data(real_image.cpu(), 1)  # Real

    def train_clients(self, epochs=5):
...
def main():
    ...
    # Initialize server and distribute data
    server = Server(num_clients, num_adversaries, num_data_per_client, device)
    server.distribute_data(dataloader, model_resnet)

    # Train clients
    server.train_clients(epochs=5)
