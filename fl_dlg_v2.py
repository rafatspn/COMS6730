...
def main():
    ...
    # Train local models
    local_models = [RealFakeClassifier().to(device) for _ in range(num_clients)]
    local_optimizers = [optim.Adam(model.parameters(), lr=0.001) for model in local_models]
    local_criteria = [nn.BCELoss() for _ in range(num_clients)]

    for client_id in range(num_clients):
        images = torch.cat(client_data[client_id])
        labels = torch.tensor(client_labels[client_id])
        train_dataset = SingleImageDataset(images, labels)
        train_loader = DataLoader(train_dataset, batch_size=4, shuffle=True)
        print(f"Training client {client_id}...")
        train(local_models[client_id], train_loader, local_criteria[client_id], local_optimizers[client_id], device, epochs=5)

    # Detect adversarial clients using RealFakeClassifier
    adversarial_clients = []
    for client_id in range(num_clients):
        test_images = torch.cat(client_data[client_id])
        test_labels = torch.tensor(client_labels[client_id])
        print(f"Testing client {client_id}...")
        test_single_images(local_models[client_id], test_images, test_labels, device)

        # Check if the client is adversarial
        reconstructed_count = sum(label == 0 for label in client_labels[client_id])
        if reconstructed_count > 0:
            adversarial_clients.append(client_id)
            print(f"Client {client_id} is detected as adversarial.")
