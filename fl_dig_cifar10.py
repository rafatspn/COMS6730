...
class RealFakeClassifier(nn.Module):
    def __init__(self):
        ...
        self.cnn = nn.Sequential(
            nn.Conv2d(3, 16, 3, stride=1, padding=1),  # Input channels updated to 3 for CIFAR-10
            nn.ReLU(),
            nn.MaxPool2d(2, 2),
        ...
        self.fc = nn.Sequential(
            nn.Linear(32 * 8 * 8, 128),  # Input size matches CIFAR-10 feature map
            nn.ReLU(),
            nn.Linear(128, 1),
            nn.Sigmoid()
        )
...
def main():
    ...
    model_resnet.fc = nn.Linear(model_resnet.fc.in_features, 10)  # CIFAR-10 has 10 classes
